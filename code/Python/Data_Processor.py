...
def process_data_file(file_path, db_manager, user_id):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            pattern = r"(\w+) (\d{2}:\d{2}:\d{2})-Temperatura=([\d,.]+);Puertas=(.*?);Luminosidad=(\d+);Movimiento=(\w+);Humedad=(\d+)"
            match = re.match(pattern, line)

            if match:
                room, time, temperature, doors, luminosity, movement, humidity = match.groups()
                ...
                db_manager.insert_sensor_reading(user_id, "temperature", temperature, room, time)
                db_manager.insert_sensor_reading(user_id, "luminosity", luminosity, room, time)
                db_manager.insert_sensor_reading(user_id, "humidity", humidity, room, time)
                db_manager.insert_sensor_reading(user_id, "movement", movement, room, time)
                if doors:  # Insertar datos de puertas solo si existen
                    ...
                    db_manager.execute_query(
                        """
                        INSERT INTO door_states (user_id, room, door_name, state, hour)
                        VALUES (?, ?, ?, ?, ?)
                        """,
                        (user_id, room, door_name, state, time)
                    )
